...
class BillController extends BaseClass {
    ...
    public function billList(){
        ...
        if($billList){
                foreach ($billList as $k=>$v){
                    $status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$v['user_id']])->get(['nickname'],true);
                    $billList[$k]['nickname'] = $status['nickname'];
                    $status = $this->table('user_address','DB_USER')->where(['is_on'=>1,'is_default'=>1,'user_id'=>$v['user_id']])->get(['name','mobile','province','city','area','street'],true);
    ...
    public function billOneDetail(){
        ...
        $where = 'A.is_on = 1';
        $order='A.id desc';
        $billOneDetail=$dataClass->getbillList(null,null,null,true,$order);
        $status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$billOneDetail['user_id']])->get(['nickname'],true);
        $billOneDetail['nickname'] = $status['nickname'];
        $status = $this->table('user_address','DB_USER')->where(['is_on'=>1,'is_default'=>1,'user_id'=>$billOneDetail['user_id']])->get(['name','mobile','province','city','area','street'],true);
