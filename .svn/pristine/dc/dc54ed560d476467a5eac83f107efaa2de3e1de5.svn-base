...
class GoodsController extends BaseClass {
    public function goodsImgUpload(){
        $goodsImg = $this->table('goods')->where(['id'=>4,'is_on'=>1])->get(['goods_album'],false);
	...
	public function goodsOneEdit()
        {  
            ...
            $data['discount_rate'] = $data['price']/$data['market_price'];       
            $goods = $this->table('goods')->where(['id'=>$data['goods_id'],'is_on'=>1])->get(['id'],true);
            if(!$goods){
                $this->R('',70009);
            }
                $goods = $this->table('goods')->where(['id'=>$data['goods_id'],'is_on'=>1])->update($data);
                if(!$goods){
                    $this->R('',40001);
                }
            $this->R(); 
    ...
    public function goodsList(){
        ...
        if($goodspage ){
            foreach ($goodspage  as $k=>$v){
                $status = $this->table('group')->where(['is_on'=>1,'goods_id'=>$v['id']])->get(['id'],false);
                $goodspage [$k]['group_num'] = count($status);
                $status = $this->table('bill')->where(['is_on'=>1,'goods_id'=>$v['id'],'status'=>2])->get(['id'],false);
