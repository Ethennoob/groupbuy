    ...
    public function groupList(){
        ...
        if($groupList){
                foreach ($groupList as $k=>$v){
                    $status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$v['leader']])->get(['nickname','user_img'],true);
                    $groupList[$k]['leader_nickname'] = $status['nickname'];
                    $groupList[$k]['leader_user_img'] = $status['user_img'];
                    $status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$v['soft']])->get(['nickname','user_img'],true);
                    $groupList[$k]['soft_nickname'] = $status['nickname'];
                    $groupList[$k]['soft_user_img'] = $status['user_img'];
                    $status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$v['stool']])->get(['nickname','user_img'],true);
                    $groupList[$k]['stool_nickname'] = $status['nickname'];
                    $groupList[$k]['stool_user_img'] = $status['user_img'];
                    $status = $this->table('group_member')->where(['is_on'=>1,'group_id'=>$v['id']])->get(['user_id'],false);
                    $groupList[$k]['join_num'] = count($status);
                    foreach ($status as $key => $user) {
                    	$status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$user['user_id']])->get(['nickname','user_img','openid'],true);
                    	$count = count($status);
                    	$v = implode(";",$status);
                    	$temp[] = $v;
                    }
