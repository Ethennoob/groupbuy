...
class GroupsController extends Baseclass {
    ...
    public function groupOneDetail(){
        ...
        $good = $this->table('goods')->where(['id'=>$groupOneDetail['goods_id']])->get(['goods_album'],true);
        $ImgUrl=explode(';', $good['goods_album']);
        ...
        $class = $this->table('group_member')->where(['is_on'=>1,'group_id'=>$id])->order('id desc');
        //查询并分页
        $userList = $this->getOnePageData($pageInfo,$class,'get','getListLength',[$file],false);
        if ($userList) {
        foreach ($userList as $k => $v) {
        	$status = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$v['user_id']])->get(['user_img'],true);
