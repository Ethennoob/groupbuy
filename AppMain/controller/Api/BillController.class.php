...
class BillController extends Baseclass {
    ...
    public function openGroupBill(){
        ...
        $rule = [
            ...
            //判断是否有这个商品
            $good = $this->table('goods')->where(['is_on'=>1,'id'=>$goods_id])->get(['id'],true);
            if(!$good){
                $this->R('',70009);
            }
            //判断是否有这个用户
            $user = $this->table('user','DB_USER')->where(['is_on'=>1,'id'=>$user_id])->get(['id'],true);
            if(!$user){
                $this->R('',70009);
            }

            //查询此用户在一元购的默认收货地址
            $address = $this->table('user_address','DB_USER')->where(['is_on'=>1,'is_default'=>1,'user_id'=>$user_id])->get(['id'],true);
            if(!$address){
                $address['id'] = null;
            }
    ...
    public function joinGroupBill(){
        ...
        $rule = [
            ...
            //判断是否有这个团
            $group = $this->table('groups')->where(['is_on'=>1,'id'=>$group_id])->get(['id'],true);
            if(!$group){
                $this->R('',70009);
            }
            //判断是否已经参与了此团
            $group_member = $this->table('group_member')->where(['is_on'=>1,'group_id'=>$group_id,'user_id'=>$user_id])->get(['id'],true);
            if($group_member){
                $this->R('',30001);
            }
            //查询此用户在一元购的默认收货地址
    ...
    public function billOneDetail(){
        ...
        if ($billOneDetail) {
            ...
            $address = $this->table('user_address','DB_USER')->where(['is_on'=>1,'id'=>$billOneDetail['address_id']])->get(null,true);
